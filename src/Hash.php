 ...
 * 利用ファイル
 * Hash.php
 * https://github.com/cakephp/cakephp/blob/2.x/lib/Cake/Utility/Hash.php
 * 
 * CakeText.php
 * https://github.com/cakephp/cakephp/blob/2.x/lib/Cake/Utility/CakeText.php
 * 
 * 作業手順
 * Hash.php の
 ...
 * 
 * CakeText.php の
 * CakeText::tokenize を Hash::tokenize として使えるように Hash.php 内にコピーする
 * 
 * Hash.php 内の
 * 「CakeText::tokenize」を「static::tokenize」に置換する
 * 
 * 複数ある throw new からcake由来を取り除く
 * 
 * throw new InvalidArgumentException(__d('cake_dev','Invalid data array to nest.'));
 * throw new \InvalidArgumentException('Invalid data array to nest.');
 * 
 * throw new InvalidArgumentException(__d('cake_dev',　'Invalid path parameter: %s, should be dot separated path or array.', var_export($path, true)));
 * throw new \InvalidArgumentException(sprintf('Invalid path parameter: %s, should be dot separated path or array.', var_export($path, true));
 * 
 * throw new CakeException(__d('cake_dev','Hash::combine() needs an equal number of keys + values.'));
